...
class PaperAnalyzer:
    ...
    def analyze_paper(self, paper: Paper) -> PaperAnalysis:
        ...
        """
        logger.info(f"Starting comprehensive LLM analysis of paper: {paper.title}")
        ...
        try:
            ...
            if self.config.enable_tag_analysis:
                ...
                primary_tags, secondary_tags = self.tag_analyzer.analyze_paper(paper)
                analysis.primary_tags = primary_tags
                analysis.secondary_tags = secondary_tags
                
                # Generate tag summary
                tag_summary = self.tag_analyzer.generate_tag_summary(primary_tags, secondary_tags)
            ...
            if self.config.enable_visual_extraction:
                ...
                analysis.key_insights.append("Visual extraction: Skipped - not implemented yet")
            ...
            if self.config.enable_qa_generation:
                ...
                analysis.key_insights.append(f"LLM Q&A Generation: {qa_summary}")
            
            # 4. Generate methodology and results summaries
            analysis.methodology_summary = self._generate_methodology_summary(paper)
            analysis.results_summary = self._generate_results_summary(paper)
            
            # Calculate analysis duration
            analysis.analysis_duration = time.time() - start_time
            
            logger.info(f"LLM analysis completed in {analysis.analysis_duration:.2f} seconds")
    ...
    def _generate_methodology_summary(self, paper: Paper) -> Optional[str]:
        ...
        methodology_keywords = ['method', 'methodology', 'approach', 'framework', 'model']
        ...
        for line in lines:
            ...
            if any(keyword in line_lower for keyword in methodology_keywords):
                if line_lower.startswith(('##', '###', '#')) or line_lower.isupper():
                    if current_section:
            ...
            if in_methodology:
                ...
                if line_lower.startswith(('##', '###', '#')) and len(line.strip()) < 100:
                    break
        ...
        if methodology_sections:
            ...
            return methodology_text[:500] + "..." if len(methodology_text) > 500 else methodology_text
    ...
    def _generate_results_summary(self, paper: Paper) -> Optional[str]:
        ...
        results_keywords = ['result', 'experiment', 'evaluation', 'performance', 'comparison']
        results_sections = []
        ...
        if results_sections:
            ...
            return results_text[:500] + "..." if len(results_text) > 500 else results_text
    ...
    def generate_analysis_report(self, analysis: PaperAnalysis) -> str:
        ...
        report_parts.append(f"**Analysis Duration:** {analysis.analysis_duration:.2f} seconds")
        report_parts.append("")
        ...
        if analysis.primary_tags or analysis.secondary_tags:
            ...
            if analysis.primary_tags:
                ...
                for tag in analysis.primary_tags:
                    report_parts.append(f"- **{tag.tag}** ({tag.value}, confidence: {tag.confidence:.2f})")
        ...
        if analysis.questions_answers:
            ...
            for category, qa_list in categories.items():
                ...
                for i, qa in enumerate(qa_list, 1):
                    report_parts.append(f"**Q{i} ({qa.difficulty_level}):** {qa.question}")
    ...
    def save_analysis(self, analysis: PaperAnalysis, output_path: Union[str, Path]) -> None:
    ...
    def get_analysis_statistics(self, analyses: list[PaperAnalysis]) -> dict:
        ...
        stats = {
            ...
            "avg_analysis_duration": sum(a.analysis_duration or 0 for a in analyses) / len(analyses),
            "tag_statistics": {},
            "qa_statistics": {}
        }
        ...
        stats["tag_statistics"] = {
            ...
            "avg_secondary_tags_per_paper": len(all_secondary_tags) / len(analyses)
        }
        ...
        for analysis in analyses:
            for qa in analysis.questions_answers:
                difficulty_counts[qa.difficulty_level] = difficulty_counts.get(qa.difficulty_level, 0) + 1
