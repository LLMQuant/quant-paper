...
class LLMQAGenerator:
    ...
    def generate_questions_answers(self, paper: Paper, 
                                 primary_tags: List = None,
                                 secondary_tags: List = None) -> List[QuestionAnswer]:
        ...
        try:
            ...
            questions_per_difficulty = max(1, self.config.num_questions // len(difficulties))
            ...
            for difficulty in difficulties:
                ...
                difficulty_qa = self._generate_difficulty_qa(paper, context, difficulty, questions_per_difficulty)
    ...
    def _prepare_context(self, paper: Paper, primary_tags: List = None, 
                        secondary_tags: List = None) -> Dict[str, Any]:
    ...
    def _generate_difficulty_qa(self, paper: Paper, context: Dict[str, Any], 
                              difficulty: str, num_questions: int) -> List[QuestionAnswer]:
        ...
        Returns:
            List of Q&A pairs
        """
        prompt = self._build_qa_prompt(paper, context, difficulty, num_questions)
    ...
    def _build_qa_prompt(self, paper: Paper, context: Dict[str, Any], 
        ...
        if difficulty == "beginner":
            ...
            categories = ["theoretical_depth", "critical_analysis", "limitations"]
        else:  # expert
            categories = ["theoretical_implications", "future_directions", "broader_impact"]
...
Respond with JSON only:"""
    ...
    def _parse_qa_response(self, qa_data: Dict[str, Any], difficulty: str) -> List[QuestionAnswer]:
    ...
    def generate_qa_summary(self, qa_pairs: List[QuestionAnswer]) -> str:
        ...
        for category, qa_list in categories.items():
            ...
            summary_parts.append(f"{category}: {len(qa_list)} questions ({', '.join(set(difficulties))})")
        
        return " | ".join(summary_parts)
    
    def generate_focused_qa(self, paper: Paper, focus_area: str, 
                          num_questions: int = 3) -> List[QuestionAnswer]:
...
{{
    ...
    def save_qa_to_file(self, qa_pairs: List[QuestionAnswer], 
